...
client_intenal = InfluxDBClient(host, port, user, password, "_internal")
...
udp_stats = list(client_intenal.query("select * from udp  where time > now() - 1m").get_points())
for udp_stat in udp_stats[:10]:
    print(udp_stat)
